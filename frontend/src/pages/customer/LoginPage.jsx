...
export default function Login() {
  ...
  const handleSocialLogin = async (provider) => {
    ...
    try {
      ...
      const result = await initiateLogin('demo.customer@nqtaxi.com', 'Demo@123');
      if (result.success) {
        const activeRole = result.user?.role || role;
        setRoleStore(activeRole);
        setAuthenticated(true);
      ...
      <div className="w-full max-w-md space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
        ...
        <Card className="p-8 space-y-6 border-primary/10 shadow-2xl shadow-primary/5">
          ...
          <div className="grid grid-cols-2 gap-4">
            <Button variant="secondary" className="py-3" onClick={() => handleSocialLogin('Google')}>
              <img src="https://www.google.com/favicon.ico" alt="Google" className="w-4 h-4 grayscale" />
              Google
            </Button>
            <Button variant="secondary" className="py-3" onClick={() => handleSocialLogin('Apple')}>
