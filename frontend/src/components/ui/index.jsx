...
export function Button({ className, variant = 'primary', loading = false, ...props }) {
  const variants = {
    primary: 'bg-primary text-primary-fg font-bold hover:opacity-90 active:scale-[0.98]',
    ...
    danger: 'bg-danger text-white font-bold hover:bg-danger/90 active:scale-[0.98]',
  };
  return (
    <button
      disabled={loading}
...
export function Card({ className, ...props }) {
  return <div className={cn('bg-surface-secondary border border-white/5 rounded-2xl p-6', className)} {...props} />;
}
    ...
    <label className={cn('flex items-center gap-3 cursor-pointer group', className)}>
      <div className="relative">
        ...
        <div className="w-5 h-5 rounded border border-white/10 bg-surface-elevated peer-checked:bg-primary peer-checked:border-primary transition-all flex items-center justify-center">
          <svg className="w-3.5 h-3.5 text-primary-fg scale-0 peer-checked:scale-100 transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="4">
